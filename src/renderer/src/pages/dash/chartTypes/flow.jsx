  ...
  return (
    <EChart style={chartStyle} opts={{ renderer:'canvas' }} option={{
      ...
      series:[{ type:'graph', layout:'force', roam:true, draggable:true,
        ...
        label:{ show:true, color:cssVar('--t1'), fontSize:9, position:'right' },
        lineStyle:{ color:cssVar('--bd2'), curveness:0.15, width:1.5 },
  ...
  return (
    <EChart style={chartStyle} opts={{ renderer:'canvas' }} option={{
      ...
      series:[{ type:'tree', data:[buildTreeData(rows, fields)], layout:'orthogonal', orient:'LR',
        ...
        symbol:'circle', symbolSize:8, label:{ position:'left', color:cssVar('--t2'), fontSize:10, align:'right' },
        leaves:{ label:{ position:'right', align:'left' } },
        lineStyle:{ color:cssVar('--bd2') }, expandAndCollapse:true, animationDuration:550,
      }],
    }} />
  )
}
  ...
  return (
    <EChart style={chartStyle} opts={{ renderer:'canvas' }} option={{
      ...
      series:[{ type:'sankey', data:nodes, links, emphasis:{ focus:'adjacency' },
        ...
        itemStyle:{ color, borderColor:cssVar('--bg') },
        label:{ color:cssVar('--t2'), fontSize:10 },
...
export function theme_river({ rows, fields, color, coresSeries, chartStyle }) {
  ...
  return (
    <EChart style={chartStyle} opts={{ renderer:'canvas' }} option={{
      ...
      legend:{ top:0, textStyle:{ color:cssVar('--t3'), fontSize:10 } },
